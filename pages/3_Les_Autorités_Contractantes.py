...
query_1 = """
MATCH (m:Marche)-[v:Vendu_par]-(A:Autorite_Contractante)
...
query_2 = """
MATCH (m:Marche)-[r]-(a:Autorite_Contractante)
...
st.title('Les Autorités Contractantes')
...
plot_1 = px.funnel(df_1,
                x=df_1['AutCont'], 
                y=df_1['nbrMarche'],
                labels={'AutCont':'Autorité Contractante', 'nbrMarche':'Nombre de Marchés'}
                )
st.plotly_chart(plot_1)

st.markdown(f'__________________________________',unsafe_allow_html=True)

st.markdown("Top 5 des Autorités avec le plus de chiffre d'affaires relié au marchés publics")
